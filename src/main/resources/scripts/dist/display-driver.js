...
export class DisplayDriver {
    ...
    drawPointer() {
        ...
        this.ctx.fillStyle = "black";
        this.ctx.fillRect(-RoadConfig.width * .05 * this.renderWidthPositionRatio, 0, ClockSize.PointerWidth, -ClockSize.PointerHeight);
        this.ctx.restore();
        this.ctx.save();
    }
    drawGameClock() {
        this.ctx.beginPath();
        this.ctx.arc((this.gameWidth * .05 * this.renderWidthPositionRatio), (this.gameHeight * .1 * this.renderHeightPositionRatio), ClockSize.OuterRadius, 0, Math.PI * 2);
        this.ctx.closePath();
        this.ctx.fillStyle = "black";
        this.ctx.fill();
        this.ctx.save();
        this.ctx.beginPath();
        this.ctx.arc((this.gameWidth * .05 * this.renderWidthPositionRatio), (this.gameWidth * .1 * this.renderHeightPositionRatio), ClockSize.InnerRadius, 0, Math.PI * 2);
        this.ctx.closePath();
        this.ctx.fillStyle = "white";
        this.ctx.fill();
        this.ctx.save();
    ...
    drawMenu() {
        ...
        this.ui.menu.forEach(b => {
            if (b instanceof Button) {
                ...
                this.ctx.fillText(b.text, (b.pos.x + b.width / 2) * this.renderWidthPositionRatio, (b.pos.y + (b.height / 3) * this.renderWidthPositionRatio) * this.renderHeightPositionRatio);
                this.ctx.save();
                this.ctx.restore();
            }
            if (b instanceof TextField) {
                ...
                this.ctx.textAlign = 'start';
                this.ctx.textBaseline = 'middle';
                this.ctx.fillText(b.text, (b.pos.x + 6) * this.renderWidthPositionRatio, (b.pos.y + (b.height / 3) * this.renderWidthPositionRatio) * this.renderHeightPositionRatio);
                this.ctx.save();
                this.ctx.save();
                this.ctx.restore();
            }
    ...
    draw() {
        ...
        switch (this.ui.state) {
            ...
            case UIStates.Menu:
                this.drawMenu();
                break;
            case UIStates.Defeated:
                this.drawCurrPlayerDefeated();
                break;
