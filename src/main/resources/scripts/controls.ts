...
export class Controls{
    ...
    is_click_targeting_castle(target: Vector, click: Vector){
        ...
        return (target.x - centerToBorderWidth <= click.x && target.x + centerToBorderWidth >= click.x &&
            target.y - centerToBorderHeight <= click.y && target.y + centerToBorderHeight >= click.y);
    }
    ...
    public mouse_down(target: Vector, castles: Array<Castle>, playerId: number){
        let orders = new Array<Castle|Vector>();
        if (this.is_targeting){
            // @ts-ignore
            let target_castle: Castle = castles.filter(castle => castle.highlighted == true).at(0);
            // @ts-ignore
            orders.push(target_castle.pos);
